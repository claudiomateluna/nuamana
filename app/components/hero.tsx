...
const Hero = () => {
  ...
  const [screenWidth, setScreenWidth] = useState(1024); // Default server value
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined') {
      ...
      setScreenWidth(window.innerWidth); // Set actual client width
    }
  ...
  const numTopImages = isClient ? (screenWidth < 460 ? 1 : (screenWidth < 1024 ? 2 : 3)) : 3;
  const numBottomImages = isClient ? (screenWidth < 460 ? 1 : (screenWidth < 1024 ? 2 : 3)) : 3;
  ...
  return (
    <div className="w-full">
      ...
      >
        ...
        <div
          className="absolute inset-0 w-full h-full bg-gradient-to-b from-(--clr5) dark:from-(--clr5) to-(--clr7)/70 dark:to-(--clr4)/70"
          style={{
            opacity: 0.8
          }}
        ></div>
        ...
        <div className={`grid grid-cols-1 sm:grid-cols-2 ${isClient && screenWidth < 1024 ? 'md:grid-cols-2' : 'md:grid-cols-3'} gap-4 z-1 mb-8 max-w-[1024px] mx-auto px-2`}>
          {topImages.slice(0, numTopImages).map((image, index) => (
            <div
              key={`top-${index}`}
              className={`border-4 rounded-lg overflow-hidden ${borderColors[index] || colors[index % colors.length]} transition-all duration-500 ease-in-out hover:scale-105 dark:mask-x-from-70% dark:mask-y-from-90%`}
              style={{
        ...
        {/* Contenido centrado */}
        <div className="relative z-1 text-center text-(--clr1) dark:text-(--dclr2) mb-8 max-w-[1024px] mx-auto px-2">
          <h2 className="text-3xl md:text-5xl font-bold text-shadow-md text-(--clr8) dark:text-(--dclr8)">
        ...
        <div className={`grid grid-cols-1 sm:grid-cols-2 ${isClient && screenWidth < 1024 ? 'md:grid-cols-2' : 'md:grid-cols-3'} gap-4 z-1 mt-8 max-w-[1024px] mx-auto px-2`}>
          {bottomImages.slice(0, numBottomImages).map((image, index) => (
            <div
              key={`bottom-${index}`}
              className={`border-4 rounded-lg overflow-hidden ${borderColors[index + topImages.length] || colors[(index + topImages.length) % colors.length]} transition-all duration-500 ease-in-out hover:scale-105 dark:mask-x-from-70% dark:mask-y-from-90%`}
              style={{
                animation: `fadeIn ${0.5 + Math.random() * 1.5}s forwards`,
                opacity: 0,
                transform: `rotate(${(Math.random() * 6) - 3}deg)`, // Rotación aleatoria entre -3 y +3 grados
