...
const SidebarDrawer = ({ isOpen, onClose }: SidebarDrawerProps) => {
  ...
  return (
    <div className="sidebar-overlay fixed inset-0 z-9999">
      ...
      {/* Drawer */}
      <div
        className={`bg-gradient-to-b from-(--clr1) to-(--clr2)/80 fixed top-0 left-0 h-screen w-[280px] sm:w-[320px] z-100 ${
          isOpen ? 'translate-x-0' : '-translate-x-full'
        }`}
      >
        <div className="flex flex-col h-full">
          {/* Header del drawer con botón de cierre */}
          <div className="p-2 border-b border-(--clr1)">
            <div className="flex justify-between items-center">
              <div className="flex items-center">
                {currentView !== 'main' && (
                  <button
                    onClick={() => setCurrentView('main')}
                    className="mr-2 p-1 rounded-full hover:bg-(--clr2)/40"
                    aria-label="Volver al menú principal"
                  >
                    <ArrowLeftIcon className="w-5 h-5 text-(--clr5)" />
                  </button>
                )}
                <div className="flex items-center">
                  <div className="px-1">
                  ...
                  <div>
                    <div className="text-(--clr4) text-xs mb-[-8px]">Guías y Scouts</div>
                    <div className="text-(--dclr7) text-xl font-bold uppercase mb-[-10px]">Nua Mana</div>
                    <div className="text-(--dclr8) text-xs font-bold">una nueva aventura</div>
                  </div>
                </div>
              </div>
              <button
                onClick={onClose}
                className="text-(--clr7) hover:text-(--clr7) focus:outline-none transition-colors"
            ...
            {currentView === 'acerca-de' && (
              <div className="ml-2 mt-2 mb-1">
                <h2
                  className="text-(--clr6) hover:text-(--dclr6) font-bold text-lg cursor-pointer transition-colors"
                  onClick={() => {
            ...
            {currentView === 'main' && (
              <>
                <nav className="space-y-1 mb-4">
                  ...
                  >
                    <IconoInicio className="w-10 h-10 mr-3 text-(--dclr6)" />
                  ...
                  <button
                    ...
                    className="flex items-center w-full p-2 text-left rounded-lg transition-all duration-200"
                  >
                    <IconoAcercaDe className="w-10 h-10 mr-3 text-(--dclr6)" />
                  ...
                  <button
                    ...
                    className="flex items-center w-full p-2 text-left rounded-lg transition-all duration-200"
                  >
                    <IconoLoQueHacemos className="w-10 h-10 mr-3 text-(--dclr6)" />
                    <span className="font-medium">Lo que hacemos</span>
                  </button>
                </nav>

                <div className="mt-auto pt-3 border-t border-(--clr5)">
                  <button
                    onClick={() => {
                      router.push('/auth/signin');
                      onClose();
                    }}
                    className="w-full mb-2 px-4 py-3 text-(--clr1) hover:text-(--clr6) bg-gradient-to-r from-(--clr6) hover:from-(--clr1) to-(--dclr6) hover:to-(--dclr2) hover:border-2 hover:border-(--clr6) rounded-[2em] font-medium transition-all duration-300"
                  >
                    Iniciar Sesión
                  </button>
                  ...
                  <div className="flex justify-center space-x-1 pt-2">
                    <a
                      ...
                      className="text-white hover:text-(--clr8) transition-colors p-2 rounded-full hover:bg-(--clr5)/50"
                    >
          ...
          {/* Footer del drawer */}
          <div className="p-2 text-center text-xs text-gray-400 border-t border-[#34495e]">
