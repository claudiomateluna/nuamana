...
const FAQItem = ({ question, answer, isOpen, toggleOpen, image }: {
  ...
  return (
    <div className="border border-(--clr4)/20 rounded-xl overflow-hidden mb-4 shadow-sm hover:shadow-md transition-shadow">
      <button
        className="w-full flex justify-between items-center p-4 text-left bg-(--clr1) dark:bg-(--clr5) hover:bg-(--clr1)/70 dark:hover:bg-(--clr5)/70 transition-colors"
        onClick={toggleOpen}
        aria-expanded={isOpen}
      >
        <h3 className="text-lg font-semibold text-(--clr4) dark:text-(--dclr2)">{question}</h3>
        {isOpen ? (
          <IconoChevronUp className="h-5 w-5 text-(--clr7) dark:text-(--dclr8) flex-shrink-0 ml-4" />
        ) : (
          <IconoChevronDown className="h-5 w-5 text-(--clr7) dark:text-(--dclr8) flex-shrink-0 ml-4" />
        )}
      </button>
      {isOpen && (
        <div className="p-4 pt-0 bg-(--clr1)/20 dark:bg-(--clr5)/60 border-t border-(--clr4)/10 dark:border-(--clr7)/30">
          <div className="flex flex-col md:flex-row gap-4 items-center">
            ...
            <p
              className="text-(--clr3) dark:text-(--clr2) flex-grow"
...
const FAQ = () => {
  ...
  return (
    <section
      className="py-15 bg-gradient-to-br from-[#f9f9f9] dark:from-(--dclr1) to-[#f5f5f5] dark:to-(--clr4) dark:to-70%"
    ...
    >
      <div className="container mx-auto">
        <div className="text-center mb-16">
          <h2
            id="faq-title"
            className="text-4xl font-bold text-(--clr4) dark:text-(--dclr6) mb-4"
          >
            Preguntas Frecuentes
          </h2>
          <p className="text-xl text-(--clr3) dark:text-(--clr2) mx-auto">
